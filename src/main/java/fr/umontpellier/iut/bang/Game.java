...
public class Game {
    ...
    public Player getCFLiurrentPlayer() {
    ...
    public int getPlayerDistance(Player player1, Player player2) {
        int indiceP1 = players.indexOf(player1);
        int indiceP2 = players.indexOf(player2);
        int distance = Math.abs(indiceP1 - indiceP2);

        if (distance == players.size() - 1){
            distance = 1;
        }
    ...
    private void updateGameFinished() {
        ...
        } else if (players.contains(sheriffPlayer) && !players.containsAll(outlawPlayers)
                && !players.contains(renegadePlayer)) { // victoire du sheriff et des adjoints
            finished = true;
            for (Player p : players) {
                winners.add(p);
            }
    ...
    public boolean samPresentEtEnVie() {
        for (Player p : players){
